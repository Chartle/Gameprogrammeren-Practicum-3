...
partial class Player : AnimatedGameObject
{
    ...
    public override void HandleInput(InputHelper inputHelper)
    {
        ...
        if (inputHelper.IsKeyDown(Keys.Left) || inputHelper.IsKeyDown(Keys.A))
            ...
            velocity.X = walkingSpeed;
        else if (!walkingOnIce && (inputHelper.KeyReleased(Keys.Right) || inputHelper.KeyReleased(Keys.D) || inputHelper.KeyReleased(Keys.Left) || inputHelper.KeyReleased(Keys.A) || isOnTheGround))
            velocity.X = 0.0f;
        if (velocity.X != 0.0f)
            Mirror = velocity.X < 0;
        if ((inputHelper.IsKeyDown(Keys.Space) || inputHelper.IsKeyDown(Keys.Up) || inputHelper.IsKeyDown(Keys.W)) && isOnTheGround)
            Jump();
    }
